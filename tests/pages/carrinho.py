...
class Carrinho(BasePage):
    ...
    def validar_carrinho(
        ...
        expect(self.label_cabecalho_descricao_produto.nth(indice_produto)).to_have_text(
            cabecalho_descricao_produto
        )
        expect(self.label_descricao_produto.nth(indice_produto)).to_have_text(
            descricao_produto
        )
        expect(self.label_preco_produto.nth(indice_produto)).to_have_text(preco_produto)
        expect(self.label_preco_total.nth(indice_produto)).to_have_text(
            preco_total_produto
        )
